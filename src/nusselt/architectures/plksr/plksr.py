...
class PLKSR(nn.Module):
    def __init__(
        self,
        dim: int = 64 ,# готово
        n_blocks: int = 28, # готово
        scale:int = 4,
        # CCM options
        ccm_type='DCCM',# готово
        # LK Options
        kernel_size: int = 17,# готово
        split_ratio: float = 0.25,# готово
        lk_type = 'PLK',# готово
        # LK Rep options
        use_max_kernel: bool = False, #готово
        sparse_kernels: Sequence[int] = [5, 5, 5, 5],# готово
        sparse_dilations: Sequence[int] = [1, 2, 3,4],# готово
        with_idt: bool = False, #not_detect
        # EA ablation
        use_ea: bool = True, # готово
        # Mobile Convert options
        is_coreml: bool = False,#not_detect
    ):
        super().__init__()
